...
async function sub(data)
{
	...
	$('#file-presentation').append('<tr id="a'+idx+'"><td>https://localhost/vote/DD/public_html/ipfs-get.php?hash='+resp.responseText.split('|')[1]+' <span onclick="files.splice('+idx+', 1);$(\'#a'+idx+'\').delete()">x</span></td></tr>');
}
		catch (e)
		{
			console.log(e)
			...
			var idx = files.length - 1;
			$('#file-presentation').append('<tr id="a'+idx+'"><td><a target="blank" href="ipfs-get.php?hash='+e.responseText+'">https://localhost/vote/DD/public_html/ipfs-get.php?hash='+e.responseText+'</a> <span onclick="files.splice('+idx+', 1);$(\'#a'+idx+'\').remove()">x</span></td></tr>');
		}
